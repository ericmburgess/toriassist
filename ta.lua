...
local debug_on = playerName == "dotproduct" and false
local version = "0.4"
...
-- predefines:
local moveSaveButtons0, moveSaveButtons1, lblSave0, lblSave1, moveButton
...
local function hookMouseUp(btn, x, y)
  ...
  for _, btn in pairs(miscButtons) do
    if os.clock()-btn.timestamp > 0.2 then btn.isMoving = false end
  end
end
...
local function hookEnterFreeze()
  ...
  for i = 1, match.round do
    ...
    if moveSaveButtons0[i] then 
      ...
      moveSaveButtons0[i].x = winWidth - 50 - 25*(math.min(10,match.round)-1-i)
    end
  end
...
moveSaveButtons0 = {}
moveSaveButtons1 = {}
lblSave1 = addLabel("Save moves 1 to", 5, 80, 1, colorBPBlue)
lblSave1.visible = false
for i = 1, 10 do
  local w, h = 22, 20
  if i<10 then label=tostring(i) else label="All" end
  moveSaveButtons1[i] = addButton(95+(w+3)*i, 80, w, h, label, function() saveSequence(1, i) end)
...
for i = 1, 10 do
  local w, h = 22, 20
  if i<10 then label=tostring(i) else label="All" end
  moveSaveButtons0[i] = addButton(0, 80, w, h, label, function() saveSequence(0, i) end)
...
CHANGELOG
  ...
  new  - framework for user-accessible settings through /ta set
